<template>
  ...
  >
    ...
    >
      <div :class="`iti-flag-small iti-flag ${selectedFlag}`" />
    </div>
    <input
      :id="id"
      ref="CountrySelector"
    ...
    >
    <div
class="country-selector__toggle" @click.stop="toggleList"
>
      <slot name="arrow">
        <svg
      ...
      >
        <v-list>
          <v-list-item-group v-model="selectedIndex" color="primary">
            <v-list-item
              v-for="(item, i) in countriesSorted"
              :key="item.iso2"
...
export default {
  ...
  methods: {
    ...
    async updateValue(val, dialCode) {
      this.tmpValue = val
      this.selectedFlag = val.toLowerCase()

      this.$emit("input", dialCode || null)
      await this.$nextTick()
      this.closeList()
    },
